...
public class AirConditioner extends MultiBlock{
	...
	protected int widthStart()	{ return -1; }
	...
	protected static final double AABB_PARAM[][] = {
			{ 1D, 1D, 0D, 16D, 15D, 15D }, 
			{ 0D, 1D, 0D, 16D, 15D, 15D }, 
			{ 0D, 1D, 0D, 15D, 15D, 15D }
	};
	...
	public boolean canSurvive(BlockState pState, LevelReader pLevel, BlockPos pPos) {
		...
		for(int iWidth = widthStart; iWidth <= widthEnd; iWidth++) {
			BlockPos iPos = pPos.offset(new BlockPos(iWidth, 0, 0).rotate(rotation).relative(facing));
			BlockState state = pLevel.getBlockState(iPos);
			if(!state.isFaceSturdy(pLevel, iPos, facing.getOpposite()))
				return false;
		}
		return true;
	}
